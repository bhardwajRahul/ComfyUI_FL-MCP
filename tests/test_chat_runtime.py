...
def test_natural_canvas_requests_and_retries_keep_canvas_controls_available():
    for request in (
        ...
        assert CANVAS_CHAT_TOOLS <= tools_for_message(request)
    ...
    assert CANVAS_CHAT_TOOLS <= turn.allowed_tools
...
def test_layout_cleanup_and_continuations_keep_the_required_tools():
    ...
    assert tools_for_message(inspection) == INSPECTION_CHAT_TOOLS
...
def test_existing_chain_edit_adds_only_the_atomic_refinement_route():
    ...
    assert {
        "get_execution_history",
        "view_output_image",
    } <= tools_for_message("Please show me this image.")
...
def test_mask_prompt_contract_stays_on_the_mask_lane():
    prompt = ren_instructions("off", {"view_node_mask", "edit_node_mask", "confirm_mask_review"})
    for rule in (
        "not proof or an expectation that a mask already exists",
        "an empty mask is valid and expected before the first paint",
        "Stop immediately on a classified failed step",
        "do not invoke GraphPatch, a planner, or diagnostic tools",
    ):
        assert rule in prompt
        assert rule not in ren_instructions("off", REFINEMENT_COMPILER_TOOLS)
